...
集成 DeepSeek AI，基于真实记录自动生成个性化分析报告。小组件连续点击 Bug 修复 + 提示词动态优化。

| 改进 | 说明 |
|------|------|
| AI 数据分析 | 调用 DeepSeek API，基于 6 个快捷区间生成分析报告（Markdown 渲染） |
| 动态提示词 | 每次分析前自动计算周末/工作日各时段差异、趋势方向、漏记检测等预处理结论 |
| 报告分享 | 报告底部「分享」按钮调系统分享面板，可发微信/QQ/复制文本 |
| 报告缓存 | 分析结果自动缓存本地，下次进入可直接查看上次报告，不消耗 API 额度 |
| 确认按钮 | 进入分析页不自动调 API，需点击「开始 AI 分析」确认后才发起请求 |
| 沉浸状态栏 | 分析页状态栏跟随主题色，深浅图标自动适配 |
| 页面美化 | 待确认卡片 + 报告卡片 + 错误/空状态卡片式布局，整体视觉统一 |
| 小组件 Bug 修复 | 连续点击卡死 + 接近零点跨天（Mutex 串行化 + 时间偏移截断） |
...
### 🤖 AI 数据分析
- 集成 **DeepSeek API**，一键生成个性化吸烟数据分析报告
- 支持 6 个快捷区间：近7天 / 14天 / 30天 / 本月 / 上月 / 全部
- 自动预处理：周末 vs 工作日各时段对比、趋势方向、漏记检测
- 报告可**分享**（微信/QQ/复制文本）或**缓存**本地（无需重复消耗 API 额度）
- 在设置页配置 API Key 后即可使用
...
| JUnit + MockK | 单元测试 |
| OkHttp | DeepSeek AI API 调用 |
| Markwon | Markdown 报告渲染 |
